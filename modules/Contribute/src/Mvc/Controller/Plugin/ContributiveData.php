...
class ContributiveData extends AbstractPlugin
{
    ...
    public function __invoke($resourceTemplate = null, ?bool $isSubTemplate = false): self
    {
        ...
        foreach ($resourceTemplate->resourceTemplateProperties() as $resourceTemplateProperty) {
            ...
            //libis - get resource query from advanced template
            $this->data['resource_query'][$term] = $rtpData->dataValue('resource_query', []);
            //end
    ...
    //libis
    public function resourceQuery(?string $term): array
    {
        if(!isset($this->data['resource_query'][$term])):
            return array();
        endif;    
        $query = array();
        parse_str($this->data['resource_query'][$term],$query);
        return $query;
    }
    //end
